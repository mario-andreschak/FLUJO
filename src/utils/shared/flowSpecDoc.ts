...
8c. Connecting a subflow also automatically offers call_subflow_* (inline result) and start_subflow_* (background task); no feature flag or invocation-mode setting is needed. For ongoing collaboration prefer start_subflow_*: the parent and child can use subflow_list, subflow_send_message, and subflow_wait to exchange progress, corrections and questions. A child addresses its parent with target "parent". Synchronous handoffs and inline calls wait for the child to return; the parent can answer during background work. Task completion is reported automatically, and a finishing parent stays available while its workers run.
