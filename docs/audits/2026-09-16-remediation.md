...
Publication follow-up: the clean-checkout inventory gate exposed a source omission after this snapshot. The local inventory included 197 routes, but Git and the isolated validation checkout contained only 196: a broad `build` ignore rule excluded the package wizard's `/api/packages/build` handler. The route was subsequently reviewed and tracked with a narrow ignore exception, and focused HTTP boundary tests were added. The recorded staging runs do not cover that previously ignored handler; clean-checkout release verification is separate evidence.
