...
import { promoteRunTodoToWorkItem } from '@/backend/services/enduringAgents';
import { EnduringAgentIdSchema } from '@/shared/types/enduringAgent';
import { assertUnlocked } from '@/utils/encryption/lockGate';
import { assertLocalRequest } from '@/utils/http/localRequest';
import { createLogger } from '@/utils/logger';

const log = createLogger('app/v1/personas/[personaId]/work-items/promote-todo/route');
...
async function POST_handler(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    return NextResponse.json(await promoteRunTodoToWorkItem(personaId, body), { status: 201 });
  } catch (error) {
    const response = personaDomainErrorResponse(error); if (response) return response;
    log.error('Failed to promote run todo', error);
    return NextResponse.json({ error: 'Failed to promote run todo.' }, { status: 500 });
  }
}
