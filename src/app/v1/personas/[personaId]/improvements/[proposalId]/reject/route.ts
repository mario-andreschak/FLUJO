...
const log = createLogger('app/v1/personas/[personaId]/improvements/[proposalId]/reject/route');
...
async function POST_handler(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    return NextResponse.json(await rejectBehaviorProposal(proposal.id, {
      actor: 'persona-owner',
      reason: 'Declined from the Persona Improvements screen.',
    }));
  } catch (error) {
    const response = personaImprovementErrorResponse(error); if (response) return response;
    log.error('Failed to decline Persona improvement', error);
    return NextResponse.json({ error: 'Could not decline this improvement.' }, { status: 500 });
  }
}
