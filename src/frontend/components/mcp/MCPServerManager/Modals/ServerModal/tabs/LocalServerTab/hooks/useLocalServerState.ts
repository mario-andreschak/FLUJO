...
export const isWebSocketConfig = (
  config: MCPServerConfig
): config is MCPWebSocketConfig => {
...
export const useLocalServerState = ({
  initialConfig,
  isOpen = true,
}: UseLocalServerStateProps) => {
    ...
    if (isOpen) {
      if (initialConfig) {
        ...
        if (initialConfig.transport === "websocket") {
          setWebsocketUrl(
            (initialConfig as MCPWebSocketConfig).websocketUrl || ""
          );
        } else {
          setWebsocketUrl("");
        }
