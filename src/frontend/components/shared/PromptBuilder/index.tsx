...
const getNodePath = (editor: Editor, node: any): Path | null => {
  try {
    // Use Editor.nodes to find the node instead of ReactEditor.findPath
...
const Element = (props: {
  ...
  const ToolReferenceComponent = () => {
    ...
    return (
      <span contentEditable={false} className="tool-reference-container">
        ...
        <span
          ...
          onClick={(e) => {
            e.preventDefault();
            e.stopPropagation();
            const path = getNodePath(editor, element);
            if (path) {
              log.debug(
                `Removing tool reference: ${toolRef.serverName}:${toolRef.toolName}`
              );
              Transforms.removeNodes(editor, { at: path });
            } else {
              log.error(
                `Could not find path for tool reference: ${toolRef.serverName}:${toolRef.toolName}`
              );
            }
          }}
          onKeyDown={(e) => {
            if (e.key === "Enter" || e.key === " ") {
              ...
              if (path) {
                log.debug(
                  `Removing tool reference via keyboard: ${toolRef.serverName}:${toolRef.toolName}`
                );
                Transforms.removeNodes(editor, { at: path });
              } else {
                log.error(
...
const Leaf = (props: {
  ...
  if (leaf.code) {
    formattedChildren = (
      <code className={leaf["tool-reference"] ? "tool-reference" : ""}>
        {formattedChildren}
      </code>
    );
  }
...
const ToolReferencePreview = ({
  ...
  useEffect(() => {
    const fetchToolInfo = async () => {
      ...
      try {
        ...
        log.error(
          `Failed to fetch tool info for ${serverName}:${toolName}`,
          error
        );
      } finally {
        setIsLoading(false);
  ...
  if (isLoading) {
    return (
      <span className="tool-reference-preview loading">{`tool:${serverName}:${toolName}`}</span>
    );
  }

  if (!toolInfo) {
    return (
      <span className="tool-reference-preview not-found">{`tool:${serverName}:${toolName} (Tool not found)`}</span>
    );
  }
  ...
  const paramsText =
    toolInfo.parameters && toolInfo.parameters.length > 0
      ? ` with parameters ${toolInfo.parameters
          .map(
            (p: any) => `\`${p.name}\` (${p.description || "No description"})`
          )
          .join(", ")}`
      : "";
  ...
  return (
    <span className="tool-reference-preview">
      [The user is referencing a tool `tool:${serverName}:${toolName}` (
      {toolInfo.description || "No description"}){paramsText}]
    </span>
  );
};
...
const PreviewRenderer = ({ value }: { value: string }) => {
  ...
  while (startIndex !== -1) {
    ...
    if (startIndex > currentIndex) {
      segments.push(
        <span key={`text-${currentIndex}`}>
          {value.slice(currentIndex, startIndex)}
        </span>
      );
    }
    ...
    if (endIndex === -1) {
      // No closing brace, treat as regular text
      segments.push(
        <span key={`text-${startIndex}`}>{value.slice(startIndex)}</span>
      );
      break;
    }
    ...
    if (fullRef.startsWith("${_-_-_") && fullRef.endsWith("}")) {
      ...
      if (parts.length >= 3) {
        ...
        log.debug(
          `Parsed tool reference for preview: ${serverName}:${toolName}`
        );
  ...
  if (currentIndex < value.length) {
    segments.push(
      <span key={`text-${currentIndex}`}>{value.slice(currentIndex)}</span>
    );
  }
    ...
    if (typeof segment === "string") {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (i < lines.length - 1) {
          ...
          paragraphs.push(
            <p key={`p-${paragraphs.length}`}>{currentParagraph}</p>
          );
          currentParagraph = [];
        }
      }
    ...
    useImperativeHandle(ref, () => ({
      insertText: (text: string) => {
        ...
        const isToolReference =
          text.startsWith("${_-_-_") && text.endsWith("}");
        log.debug("Checking if text is a tool reference", {
          isToolReference,
          text,
        });
        ...
        if (isToolReference) {
          ...
          if (parts.length >= 3) {
            ...
            if (editor.selection) {
              ...
              log.debug(
                `No selection, inserting tool reference at end of document`
              );
