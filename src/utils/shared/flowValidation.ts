...
/**
 * Remove every tool/resource pill belonging to one of `servers` from a prompt template,
 * then tidy the whitespace the removed pills leave behind (pills are inserted with a
 * leading space). Pure string transform — used to drop dead tool references.
 */
export function stripServerBindings(text: string, servers: Set<string>): string {
  const matches = findBindings(text).filter((b) => servers.has(b.server));
  if (matches.length === 0) return text;
  // Remove from the end so earlier match indices stay valid.
  let out = text;
  for (const m of [...matches].sort((a, b) => b.index - a.index)) {
    out = out.slice(0, m.index) + out.slice(m.index + m.fullMatch.length);
  }
  return out.replace(/[ \t]{2,}/g, ' ').replace(/[ \t]+$/gm, '');
}

export interface PromptCleanup {
  nodeId: string;
  promptTemplate: string;
}

/**
 * When the given nodes are about to be removed, compute the prompt-template edits needed to
 * drop now-dead tool references from the Process nodes that survive.
 *
 * Deleting an MCP node removes its edges (so the runtime tool wiring self-heals), but the
 * binding pills already embedded in a connected Process node's prompt linger and point at a
 * server the node can no longer reach. This strips exactly those pills — and only for
 * servers that are no longer connected to that Process node after the removal, so a server
 * still provided by another MCP node keeps its pills.
 *
 * Returns one entry per affected Process node (only when its prompt actually changes).
 */
export function computeOrphanedPromptCleanups(
  ...
  // Only matters when an MCP node bound to a server is being removed.
  const removingBoundMcp = nodes.some(
    (n) => removedSet.has(n.id) && getNodeType(n) === 'mcp' && n.data?.properties?.boundServer
  );
  if (!removingBoundMcp) return [];

  const survivingNodes = nodes.filter((n) => !removedSet.has(n.id));
  const survivingEdges = edges.filter((e) => !removedSet.has(e.source) && !removedSet.has(e.target));

  const cleanups: PromptCleanup[] = [];
  for (const node of survivingNodes) {
    if (getNodeType(node) !== 'process') continue;
    const prompt = node.data?.properties?.promptTemplate;
    if (typeof prompt !== 'string' || !prompt) continue;

    const before = mcpServersConnectedToProcess(node.id, nodes, edges);
    const after = mcpServersConnectedToProcess(node.id, survivingNodes, survivingEdges);
    const toStrip = new Set([...before].filter((s) => !after.has(s)));
    if (toStrip.size === 0) continue;

    const cleaned = stripServerBindings(prompt, toStrip);
    if (cleaned !== prompt) cleanups.push({ nodeId: node.id, promptTemplate: cleaned });
  }
  return cleanups;
}
