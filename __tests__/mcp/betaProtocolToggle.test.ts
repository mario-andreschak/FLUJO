...
describe('isMcpBetaProtocolEnabled', () => {
  ...
  it('is enabled when the flag is set', async () => {
    loadItemMock.mockResolvedValue({ experimental: { enabled: true, mcpBetaProtocol: true } });
...
describe('shouldRecreateClient — beta toggle flips', () => {
  it('rebuilds a v1 client when the beta protocol is enabled', () => {
    const config = stdio();
    const client = withTransport(createNewClient(config), createTransport(config));
  ...
  it('rebuilds a beta client when the beta protocol is disabled again', () => {
    const config = stdio();
    const client = withTransport(createNewBetaClient(config), createBetaTransport(config));
...
describe('shouldRecreateClient — stable beta connections', () => {
  ...
  it('rebuilds a beta stdio client when the spawn parameters changed', () => {
    const client = withTransport(createNewBetaClient(stdio('node')), createBetaTransport(stdio('node')));
...
describe('beta factories', () => {
  ...
  it('stashes the SAME stdio config key as the v1 transport factory', () => {
    const config = stdio();
    const beta = createBetaTransport(config) as unknown as { __flujoStdioKey?: string; __flujoKind?: string };
    const v1 = createTransport(config) as unknown as { __flujoStdioKey?: string };
  ...
  it('stashes kind + http key on streamable transports and refuses websocket', () => {
    const beta = createBetaTransport(streamable()) as unknown as { __flujoHttpKey?: string; __flujoKind?: string };
    expect(beta.__flujoKind).toBe('streamable');
    expect(typeof beta.__flujoHttpKey).toBe('string');

    expect(() =>
      createBetaTransport({ name: 'ws', transport: 'websocket', websocketUrl: 'ws://x' } as unknown as MCPServerConfig)
    ).toThrow(/websocket/i);
  });
});
