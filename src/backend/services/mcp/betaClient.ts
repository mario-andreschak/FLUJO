...
export async function isMcpBetaProtocolEnabled(): Promise<boolean> {
  try {
    ...
    log.warn('Failed to read mcpBetaProtocol setting; defaulting to disabled', { err });
    return false;
  }
}
...
export function createBetaTransport(
...
): BetaStdioClientTransport | BetaStreamableHTTPClientTransport | BetaSSEClientTransport {
  if (config.transport === 'websocket') {
    throw new Error('The v2-beta MCP SDK has no websocket transport; use the v1 path');
  }
  ...
  if (config.transport === 'streamable') {
    ...
    if (streamableConfig.oauthClientId || streamableConfig.oauthClientInformation) {
      ...
      options.authProvider = createOAuthClientProvider(streamableConfig) as unknown as BetaOAuthClientProvider;
    }
    const transport = new BetaStreamableHTTPClientTransport(new URL(streamableConfig.serverUrl), options);
  ...
  if (config.transport === 'sse') {
    ...
    const transport = new BetaSSEClientTransport(new URL(sseConfig.serverUrl), options);
    const keyed = transport as unknown as TransportWithConfigKey;
    keyed.__flujoHttpKey = httpConfigKey(config);
