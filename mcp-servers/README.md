...
The browser server is seeded disabled by default. Enable it in MCP Manager, or set `FLUJO_BROWSER_ENABLED=1` before first startup to seed it enabled. Its shipped record enables MCP Apps so the browser view is immediately available once the server is enabled; disabling the server still blocks both app resource access and tool calls. The browser package's npm `install` lifecycle uses `scripts/install-browser.mjs` to download the Patchright-version-matched managed Chromium. Standalone package installs still provision it automatically. The FLUJO installers defer only this lifecycle during `npm ci`, then run it as the explicit, diagnosable `patchright-chromium` stage; all other dependency lifecycle scripts remain enabled. The Docker image also installs the required Linux libraries and uses a shared browser cache readable by its unprivileged runtime user. Installations that deliberately suppress npm lifecycle scripts with `--ignore-scripts` must run `npx --no-install patchright install chromium` themselves after dependencies exist.

Browser provisioning classifies TLS trust, proxy authentication, DNS, timeout, OS-dependency, and filesystem failures while preserving the nonzero exit code. Diagnostics redact common credentials/tokens, and both installers provide a sanitized log path. After fixing the network or trust configuration, retry only the browser stage from the FLUJO directory with `npm run install --workspace=@mario.andreschak/mcp-browser`. Patchright supports current Windows, macOS, and Linux platforms covered by its Chromium distribution; failures to launch are returned as a stable `BROWSER_UNAVAILABLE` MCP error.
